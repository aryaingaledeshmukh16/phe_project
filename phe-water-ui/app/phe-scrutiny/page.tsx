...
const CURRENT_USER_NAME = "Venkatesh Choubey";
...
export default function PHEScrutinyPage() {
  ...
  const currentUserName = getStoredUserName() || CURRENT_USER_NAME;
  ...
  const filteredApplications = useMemo(() => {
    if (activeTab === "document") {
      return applications.filter((app) => {
        ...
        return status === "deputy engineer verification completed" || status === "" || status === "pending";
      });
    }
  ...
  async function saveScrutiny() {
    ...
    if (!remark.trim()) {
      alert("Remark is required before saving PHE verification.");
      return;
    }
    ...
    try {
      ...
      alert("PHE verification updated successfully.");
      closePanel();
    } catch (err) {
      console.error(err);
      alert(err instanceof Error ? err.message : "PHE verification update failed.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="je-page">
      <header className="je-header-card">
        <div className="je-header-content only-user-header">
          <div className="je-heading user-only-heading">
            <div className="je-role">Role: {CURRENT_ROLE}</div>
            <div className="je-welcome">Welcome {currentUserName}</div>
          </div>
        </div>
      </header>

      <nav className="je-navigation">
        <button type="button" className={`je-nav-item ${activeTab === "document" ? "active" : ""}`} onClick={() => setActiveTab("document")}>PHE Verification</button>
        ...
        {!activePanel ? (
          <section className="je-card">
            <div className="je-section-title">
              {activeTab === "document" ? "PHE Verification" : "Application Status"}
            </div>
          ...
          <section className="details-card">
            {activePanel === "details" && selectedApplication && (
              <>
                ...
                <div className="details-section-title">PHE Verification</div>
