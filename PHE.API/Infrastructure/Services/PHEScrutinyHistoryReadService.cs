...
{
    public class PHEScrutinyHistoryReadService : IPHEScrutinyHistoryReadService
    {
        private readonly ApplicationDbContext _context;

        public PHEScrutinyHistoryReadService(ApplicationDbContext context)
        {
            _context = context;
        }

        public async Task<List<PHEHistoryItem>> GetByApplicationNoAsync(string applicationNo, CancellationToken cancellationToken = default)
        {
            return await _context.ApplicantsLogs
                .AsNoTracking()
                .Where(x => x.ApplicationNo == applicationNo)
                .OrderByDescending(x => x.LogId)
                .Select(x => new PHEHistoryItem
                {
                    logId = x.LogId,
